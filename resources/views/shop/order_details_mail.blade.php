...
<div class="row">
   <h3>Total : <span style="font-size: small; color: green">$ {{$order->payedTotal}}</span></h3>
</div>
<hr>
@foreach($order->products as $product)
    <div class="row">
        <h4>Product name : <span style="font-size: small;">{{ $product->name  }}</span> </h4>
        <h4>Product price : <span style="font-size: small; color: green">$ {{ $product->price  }}</span></h4>
        ...
        <h4>Total : <span style="font-size: small; color: green;">$ {{ $product->pivot->quantity *  $product->price}}</span></h4>
        <hr>
    </div>
@endforeach
